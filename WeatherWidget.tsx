    ...
    <div className="bg-white border-2 border-black p-4 rounded-2xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col md:flex-row items-center gap-6 min-w-fit">
      ...
      <div className="flex flex-col gap-2">
        <div className="hidden md:grid grid-cols-2 gap-x-4 gap-y-1 text-[10px] font-bold text-gray-500 uppercase tracking-tight">
          <div><span className="text-gray-300">Hum:</span> {weather.humidity}</div>
          <div><span className="text-gray-300">Viento:</span> {weather.wind}</div>
          <div className="col-span-2 text-blue-600 truncate max-w-[180px]">{weather.forecast}</div>
        </div>
        
        <div className="flex gap-3 mt-1 pt-1 border-t border-gray-100 md:border-t-0">
          <a href="https://www.aemet.es/es/eltiempo/prediccion/municipios/leon-id24089" target="_blank" rel="noopener noreferrer" className="text-[9px] font-black text-blue-500 hover:underline uppercase">AEMET</a>
          <a href="https://www.eltiempo.es/leon.html" target="_blank" rel="noopener noreferrer" className="text-[9px] font-black text-blue-500 hover:underline uppercase">ELTIEMPO.ES</a>
        </div>
      </div>
    </div>
  );
